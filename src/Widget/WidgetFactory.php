...
class WidgetFactory {
  ...
  public function create(Field $field, mixed $current, array $answers = []): WidgetInterface {
    $widget = match ($field->type) {
      ...
      FieldType::FilePicker => new FilePickerWidget($field->pickerStart, is_string($current) ? $current : '', $field->pickerMode, $field->pickerExtensions, $field->pickerShowHidden, pageSize: $field->pageSize),
      FieldType::MultiFilePicker => new FilePickerWidget($field->pickerStart, Field::stringList($current), $field->pickerMode, $field->pickerExtensions, $field->pickerShowHidden, multiple: TRUE, pageSize: $field->pageSize),
