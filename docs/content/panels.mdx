...
Fullscreen affects only the interactive TUI; headless collection ignores it. The terminal width is read once at start-up, so a window resized mid-session reflows on height but keeps its width until the next run.
