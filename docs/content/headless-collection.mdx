...
For automation and AI agents, the form describes itself - `schema()`, `agentHelp()` and `validate()` let a tool discover and drive it without prompting. See [AI agents](/ai-agents) for the full surface, with the runnable scripts in [`playground/05-headless/`](https://github.com/drevops/tui/tree/main/playground/05-headless).
