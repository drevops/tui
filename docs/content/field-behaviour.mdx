...
`->validate(Ripeness::validate(...))` - or let the engine discover it: registering
a namespace (`new Tui($form, handler_namespaces: ['App\\Handler'])`) resolves the class by field id
(`red_apple` -> `RedApple`) and uses its static `validate()`/`transform()`
...
Both declaration styles are runnable in
[`playground/07-field-behaviour/`](https://github.com/drevops/tui/tree/main/playground/07-field-behaviour).
