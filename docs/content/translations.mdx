...
A runnable demo in [`playground/13-translations/`](https://github.com/drevops/tui/tree/main/playground/13-translations) localizes chrome and questions into Ukrainian - using the shipped `uk.php` for chrome plus a small local catalog for the form's own labels - and exercises pluralized rendering in its multi-select summary.
