...
A message whose wording depends on a count - "1 item selected" versus "5 items selected" - is rendered with `Translator::formatPlural($count, $singular, $plural)`. The count picks the form and `@count` is bound to it:
