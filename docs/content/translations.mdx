...
A runnable demo in [`playground/13-translations.php`](https://github.com/drevops/tui/blob/main/playground/13-translations.php) localizes chrome and questions into Ukrainian - using the shipped `uk.php` for chrome plus a small local catalog for the form's own labels - and exercises pluralized rendering in its multi-select summary.
