...
Some strings carry `@name` placeholders (for example `Enter a number @constraint.` or `@count selected`). Keep each placeholder verbatim in the translation - the library substitutes the value at render time.
...
A Spanish catalog localizing chrome and questions alike is runnable in [`playground/13-translations/`](https://github.com/drevops/tui/tree/main/playground/13-translations).
