---
title: Contributing
description: 'How to set up the project locally, run the linters and tests, and send changes - issues and pull requests welcome.'
keywords: ['contributing', 'development', 'tests', 'linting', 'pull request']
---
...
Tests are written with PHPUnit 11 and live under `tests/phpunit/Unit/`, organized by component, with shared fixtures in `tests/phpunit/Fixtures/` and utilities in `tests/phpunit/Traits/`. Filesystem seams use vfsStream, so the suite needs no real I/O.
...
Prefer data providers, cover new behavior with both unit and functional tests where it makes sense, and keep coverage high.
