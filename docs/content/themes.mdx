...
Each adaptive theme below is shown in its four looks: the dark and light palette, each without a frame and inside the rounded [border](/panels#bordered-panels) (`['border' => 'rounded']`). Every adaptive theme has a runnable script in [`playground/09-themes-*`](https://github.com/drevops/tui/tree/main/playground).
...
class AquaTheme extends DefaultTheme {
  public function title(string $text): string {
    $style = $this->isDark
      ? Sgr::of(Sgr::Bold, Sgr::Cyan)
      : Sgr::of(Sgr::Bold, Sgr::Blue);

    return $this->paint($style, $text);
  }

  public function value(string $text, bool $selected = FALSE): string {
    $style = $this->isDark ? Sgr::of(Sgr::Sky) : Sgr::of(Sgr::Cobalt);

    return $this->paint($this->emphasize($style, $selected), $text);
  }
}
```
