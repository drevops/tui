...
Or register a short alias with `ThemeManager::register('aqua', AquaTheme::class)`, then `->theme('aqua')`. The [playground's ocean theme](https://github.com/drevops/tui/tree/main/playground) goes further, overriding many atoms and `render*()` methods for a distinct look with a start banner:
