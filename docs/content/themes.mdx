...
Each adaptive theme below is shown in its four looks: the dark and light palette, each without a frame and inside the rounded [border](/panels#bordered-panels) (`['border' => 'rounded']`). Every adaptive theme has a runnable script in [`playground/09-themes/`](https://github.com/drevops/tui/tree/main/playground/09-themes).
...
Or register a short alias with `ThemeManager::register('aqua', AquaTheme::class)`, then `->theme('aqua')`. The [playground's ocean theme](https://github.com/drevops/tui/tree/main/playground/09-themes) goes further, overriding many atoms and `render*()` methods for a distinct look with a start banner:
