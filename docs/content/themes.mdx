...
Each adaptive theme below is shown in its four looks: the dark and light palette, each inside the default rounded [border](/panels#bordered-panels) and with the frame explicitly stripped (`['border' => 'none']`). Every adaptive theme has a runnable script in [`playground/09-themes-*`](https://github.com/drevops/tui/tree/main/playground).
