...
$form = Form::create('My form')
  ->panel('general', 'General', function (PanelBuilder $p): void {
    ...
    $p->text('slug', 'Slug')
      ->derive(new Derive('{{name}}', transform: 'machine'));

    $p->select('category', 'Category')
      ->default('fruit')
      ->options(['fruit' => 'Fruit', 'vegetable' => 'Vegetable']);

    // Shown only when the condition holds; compose with
...
$p->text('label', 'Label')
  ->derive(new Derive('{{category}}/{{slug}}', 'lower'));
...
```php
$p->text('heat_level', 'Heat level')->default('mild')
  ->when(new Condition('category', eq: 'vegetable'));
$p->confirm('warning_label', 'Add a heat warning?')->when(Condition::all(
  new Condition('category', eq: 'vegetable'),
  new Condition('heat_level', ne: 'mild'),
));
```
