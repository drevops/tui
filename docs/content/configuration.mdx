...
[Themes](/themes)), `->keys()` sets the key bindings (see
[Key bindings](/key-bindings)), `->footer()` toggles the key-hint footer,
`->clearOnExit()` keeps or clears the final frame, `->color()` / `->unicode()`
force a [display mode](/display-modes), and `->translator()` presents chrome and
questions in another language (see [Translations](/translations)).
...
```php
$p->text('heat_level', 'Heat level')->default('mild')->when(new Condition('tags', contains: 'spicy'));
$p->confirm('warning_label', 'Add a heat warning?')->when(Condition::all(new Condition('tags', contains: 'spicy'), new Condition('category', eq: 'vegetable')));
```
