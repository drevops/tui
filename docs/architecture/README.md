...
No widget extends another widget. Each one composes its behaviour from capability interfaces (`OptionsCapableInterface`, `SelectionCapableInterface`, `MultiSelectionCapableInterface`, `FilterCapableInterface`, `SearchCapableInterface`, `PagingCapableInterface`, `TextEditCapableInterface`, `CompletionCapableInterface`, `StepCapableInterface`, `RevealCapableInterface`, `ExternalEditCapableInterface`) and the traits that implement them, so the controller and the tests interact with a capability - "is this widget filterable, can it page" - rather than a concrete class.
